...
namespace UnityEngineAnalyzer.Variable
{
    ...
    public class PrivateFieldNeverUsedAnalyzer : DiagnosticAnalyzer
    {
        public override ImmutableArray<DiagnosticDescriptor> SupportedDiagnostics => ImmutableArray.Create(DiagnosticDescriptors.PrivateFieldNeverUsed);
        ...
        {
            // check for exclusion
            if (context.IsExcluded(DiagnosticIDs.PrivateFieldNeverUsed))
            {
                return;
            }
                ...
                .OfType<VariableDeclaratorSyntax>())
            {
                ...
                if (nodeSymbol != null && nodeSymbol.DeclaredAccessibility == Accessibility.Private)
                {
            ...
            if (symbolHashSet.Count != 0)
            {
                ...
                {
                    context.ReportDiagnostic(Diagnostic.Create(DiagnosticDescriptors.PrivateFieldNeverUsed, symbol.Locations[0], symbol.Name));
                }
            }
        }
    }
